...
/**
 * CSV ingestion in a dataframe. <br/>
 * Reads a JSON file called Restaurants_in_Durham_County_NC.json, stores it in a dataframe
 *
 * @author jgp
 */
public class JsonIngestionSchemaManipulationApp {
    ...
    private void start() {
        ...
        Dataset<Row> df = spark.read().json("data/Restaurants_in_Durham_County_NC.json").cache();
        System.out.println("*** Right after ingestion");
        df.show(5);
        df.printSchema();
        System.out.println("We have " + df.count() + " records.");

        // lit(): 用于创建包含常量值的列，以在数据集中添加新的列。
        // 它通常用于将常量值添加到每一行中，以便进行后续的操作或计算。
        // lit 函数返回一个Column类型的对象，可以将其与其他列表达式一起使用。
        //@formatter:off
        ...
        //@formatter:on
        // concatenate multiple columns into one column: id
